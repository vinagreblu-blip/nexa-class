// Renderizador de Histórico Acadêmico para a FATECE baseado em HTML + CSS.
...
// de verdade compartilhada por TODOS os cursos da FATECE.
...
.header-title { flex: 1; padding-top: 1mm; text-align: center; }
.header-title h1 { font-size: 20px; line-height: 1; font-weight: 700; }
.header-title h2 { margin-top: 7px; font-size: 12px; line-height: 1.1; font-weight: 700; }
.institution-contact {
  width: 100%; margin-top: 1px;
  text-align: center; font-size: 7px; line-height: 1.3;
}
.institution-contact p { margin-bottom: 1px; }
.institution-contact em { font-style: italic; }
.institution-contact a, a { color: #0066aa; text-decoration: underline; }

/* ===== Identificação do aluno ===== */
.student-grid {
  display: grid;
  grid-template-columns: 12% 56% 15% 17%;
  width: 100%;
  margin-top: 5px;
...
.course-field:nth-child(2) .course-content { text-align: center; font-size: 10px; }
.course-field:nth-child(3) .course-content { text-align: left; font-size: 9px; line-height: 1.35; }
.course-field:nth-child(3) .course-content p { margin-bottom: 4px; }
...
.academic-table td:first-child { text-align: center; vertical-align: middle; }
...
.academic-table tbody td:nth-child(7) { text-align: center; font-style: normal; }
...
.academic-table .period-total td:nth-child(4) { text-align: right; padding-right: 5px; }

/* ===== Resumo carga horária / média ===== */
.academic-summary {
  width: 70%;
  margin: 7mm auto 0;
  color: #111;
...
.summary-average { text-align: center; }
.summary-value { text-align: center; font-size: 9px; font-weight: 700; }
...
.signature-line { width: 65%; height: 0; margin: 0 auto 3px; border-top: 1px solid #000; }
...
.footer p { margin: 0; }
.footer a { color: #0066aa; text-decoration: underline; }
...
/* ===== Quebra interna ===== */
.academic-table tr, .activities-table tr { page-break-inside: avoid; break-inside: avoid; }
.academic-summary, .activities-summary, .signature, .footer, .verificacao {
...
.activities-table { width: 100%; border-collapse: collapse; table-layout: fixed; page-break-inside: avoid; break-inside: avoid; }
.activities-table tr { page-break-inside: avoid; break-inside: avoid; }
.activities-table th, .activities-table td { border-color: #111 !important; }

.enade-bar { background: #b7b7b7 !important; }
.student-grid, .course-grid, .academic-grid { border-color: #111 !important; }
.student-grid .field, .course-grid .course-field, .academic-grid .academic-field { border-color: #111 !important; }
...
function renderHeader(faculdade: FaculdadeInfo, logoDataUrl: string | null): string {
  ...
  return `<header class="header">
    ...
    <div class="institution-contact">
      <p>CNPJ ${esc(faculdade.cnpj)} / E-mail.: <a href="mailto:${esc(faculdade.email)}">${esc(faculdade.email)}</a> / ${esc(faculdade.telefone)}</p>
      <p>ENDEREÇO: ${esc(faculdade.endereco)}</p>
    </div>
  </header>`;
}

function renderStudentGridCompleta(aluno: Aluno): string {
  return `<section class="student-grid">
    <div class="field"><label>CGA</label><div class="field-value">${esc(aluno.matricula || '')}</div></div>
    <div class="field"><label>NOME DO ALUNO</label><div class="field-value">${esc(aluno.nome || '')}</div></div>
    <div class="field"><label>SEXO</label><div class="field-value">${esc(aluno.sexo || '')}</div></div>
    <div class="field"><label>DATA DE NASC</label><div class="field-value">${esc(formatarDataOuVazio(aluno.data_nascimento))}</div></div>
  </section>
  <section class="student-grid second-row">
    ...
    <div class="field"><label>ORGÃO EMISSOR</label><div class="field-value">${esc(aluno.orgao_emissor || '')}</div></div>
...
function renderRegulatorio(regulatory: string): string {
  ...
  return sentences
    .map((s) => `<p>${esc(s.trim())}${s.trim().endsWith('.') ? '' : '.'}</p>`)
    .join('');
}
...
function renderSummary(disciplinas: HistoricoDisciplina[]): string {
  ...
  const notas = disciplinas
    .map((d) => parseNota(d.nota))
    .filter((n): n is number => n !== null);
  const media = notas.length
    ? (notas.reduce((s, n) => s + n, 0) / notas.length).toFixed(1).replace('.', ',')
    : '';
  return `<section class="academic-summary">
    <div class="summary-header">
      <div class="summary-total">CARGA HORÁRIA TOTAL: ${fmtNum(chTotal) || ''}H</div>
      <div class="summary-average">MÉDIA GERAL</div>
      <div class="summary-value">${esc(media)}</div>
    </div>
  </section>`;
}
...
function renderActivities(disciplinas: HistoricoDisciplina[]): string {
  ...
  const isEstagio = (d: HistoricoDisciplina) =>
    /EST[ÁA]GIO\s+(CURRICULAR\s+)?SUPERVISIONADO/i.test(d.disciplina || '');
  ...
  const chCurso = disciplinas
    .filter((d) => !isAtividade(d) && !isEstagio(d))
...
function renderFooter(faculdade: FaculdadeInfo): string {
  const email = faculdade.email || 'secretaria@faculdadefatece.com.br';
  const telefone = faculdade.telefone || '(71) 9 2003-7875';
  return `<footer class="footer">
    <p>QUALQUER INFORMAÇÃO, DEVE SER SOLICITADA ATRAVÉS DO NOSSO E-MAIL: <a href="mailto:${esc(email)}">${esc(email)}</a>,</p>
    <p>OU ATRAVÉS DO WHATSAPP ${esc(telefone)}.</p>
  </footer>`;
}
...
export function gerarHtmlFatece(opts: FateceHtmlOpts): string {
  const {
    aluno,
    disciplinas,
    faculdade,
  ...
  // PÁGINA 1 — cabeçalho, identificação completa, curso, acadêmico, ENADE e tabela
  const pagina1 = `<div class="page">
    ${renderHeader(faculdade, logoDataUrl)}
    ${renderStudentGridCompleta(aluno)}
  ...
  // PÁGINA 2 — resumo, atividades, assinatura, rodapé e verificação
...
export interface FateceRenderOpts {
  aluno: Aluno;
  disciplinas: HistoricoDisciplina[];
  faculdade: FaculdadeInfo;
  cursoInfo: CursoInfo | null;
...
export async function renderHtmlFatecePdf(opts: FateceRenderOpts): Promise<void> {
  const {
    aluno,
    disciplinas,
    faculdade,
  ...
  const nomeSignatario = assinatura?.nome_signatario || faculdade.diretor || 'José Augusto Maciel Torres';
