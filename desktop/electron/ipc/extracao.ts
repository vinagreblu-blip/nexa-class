...
function capitalizar(nome: string): string {
  ...
  return nome.trim().split(/\s+/).map((p) => {
    ...
    if (pequenas.includes(l)) return l;
...
function corrigirOCRNumerico(s: string): string {
  return s
    .replace(/[OoDQ]/g, '0')
    .replace(/[Iil|]/g, '1')
    .replace(/[Zz]/g, '2')
...
function corrigirOCR(s: string): string {
  return s
...
function parseTexto(textoOriginal: string): DadosExtraidos {
  const r: DadosExtraidos = {
  ...
  if (rgRegiaoIdx >= 0) {
    ...
    if (orgMatch) r.orgaoEmissor = orgMatch[1].toUpperCase();
    ...
    for (let li = 1; li < Math.min(4, linhasApos.length); li++) {
      ...
      if (/CPF|DATA|NASC|FILIA|NACION/i.test(linhaDados)) continue;
    ...
    if (!r.naturalidade) {
      ...
      if (ufM && estados.has(ufM[1].toUpperCase())) r.naturalidade = ufM[1].toUpperCase();
      ...
      if (ufCorrigir) {
        ...
        if (estados.has(uf)) r.naturalidade = uf;
      }
    }
  }
  ...
  if (!r.rg) {
    ...
    if (rgM && rgM[1].replace(/\D/g, '').length >= 5) r.rg = rgM[1].trim();
  }

  // === Nacionalidade ===
  if (/BRASILEIR[AO]/i.test(textoOriginal)) r.nacionalidade = 'Brasileiro(a)';

  // === Sexo ===
  const sexoM = textoOriginal.match(/SEXO\s*[:\n]?\s*(MASC|FEM)/i);
  if (sexoM) r.sexo = /F/i.test(sexoM[1]) ? 'Feminino' : 'Masculino';

  // === Naturalidade ===
  if (!r.naturalidade) {
    const natM = textoOriginal.match(/(?:NATURALIDADE|UF)\s*[:\n]?\s*([A-Z]{2})\b/i);
    if (natM && estados.has(natM[1].toUpperCase())) r.naturalidade = natM[1].toUpperCase();
  }
...
async function extrairDadosDocumento(
...
): Promise<ApiResult<DadosExtraidos>> {
  ...
  if (ext === 'pdf') {
    ...
    if (!textoCompleto.match(/\d{3}\.\d{3}\.\d{3}/) && !textoCompleto.match(/NOME/i)) {
      ...
      const tempDir = path.join(app.getPath('temp'), 'nexa-ocr');
...
export function registrarExtracaoHandlers(): void {
  ipcMain.handle(IPC_CHANNELS.EXTRAIR_DADOS_DOC, requerAuth(extrairDadosDocumento));
}
