...
exports.registrarHistoricoHandlers = registrarHistoricoHandlers;
...
function formatarNome(s) {
    ...
    if (/[a-zà-ÿ]/.test(s) && s !== s.toUpperCase())
        return s;
    ...
    return s.toLowerCase().split(/\s+/).map((p, i) => {
        if (i > 0 && pequenas.has(p.replace(/[^\wÀ-ÿ-]/g, '')))
            return p;
...
async function registrarNoWeb(codigo, hash, aluno, emitidoEm) {
    try {
        ...
        const resp = await fetch(url, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-api-key': config_1.CONFIG.VERIFICACAO_API_KEY },
            body: JSON.stringify({
...
function validarDisc(d) {
    if (!d.periodo?.trim())
        return 'Período é obrigatório';
    if (!d.disciplina?.trim())
        return 'Disciplina é obrigatória';
    return null;
}
function proximaOrdem(alunoId, periodo) {
    const db = (0, database_1.getDb)();
    const row = db
        .prepare('SELECT COALESCE(MAX(ordem),0) AS maxOrdem FROM historico_disciplinas WHERE aluno_id = ? AND periodo = ?')
        .get(alunoId, periodo);
    return (row?.maxOrdem ?? 0) + 1;
}
function criar(_event, alunoId, input) {
    ...
    const info = db
        .prepare(`INSERT INTO historico_disciplinas (aluno_id, periodo, disciplina, docente, titulacao, ch, nota, status, ordem)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(alunoId, input.periodo.trim(), input.disciplina.trim(), input.docente?.trim() || null, input.titulacao?.trim() || null, input.ch?.trim() || null, input.nota?.trim() || null, input.status?.trim() || null, ordem);
    const row = db
...
function atualizar(_event, id, input) {
    ...
    const db = (0, database_1.getDb)();
    const result = db
        .prepare(`UPDATE historico_disciplinas
       SET periodo = ?, disciplina = ?, docente = ?, titulacao = ?, ch = ?, nota = ?, status = ?
       WHERE id = ?`)
        .run(input.periodo.trim(), input.disciplina.trim(), input.docente?.trim() || null, input.titulacao?.trim() || null, input.ch?.trim() || null, input.nota?.trim() || null, input.status?.trim() || null, id);
    if (result.changes === 0)
        return { ok: false, error: 'Disciplina não encontrada' };
    const row = db.prepare('SELECT * FROM historico_disciplinas WHERE id = ?').get(id);
    return { ok: true, data: row };
}
...
async function gerarPdf(event, alunoId) {
    ...
    const info = (0, faculdades_1.getFaculdadeInfo)(aluno.faculdade);
...
function registrarFontes(doc) {
    const dir = process.env.WINDIR ? node_path_1.default.join(process.env.WINDIR, 'Fonts') : 'C:\\Windows\\Fonts';
    const reg = node_path_1.default.join(dir, 'calibri.ttf');
    const bold = node_path_1.default.join(dir, 'calibrib.ttf');
    const ital = node_path_1.default.join(dir, 'calibrii.ttf');
    if (node_fs_1.default.existsSync(reg) && node_fs_1.default.existsSync(bold)) {
        try {
            doc.registerFont('Calibri', reg);
            doc.registerFont('Calibri-Bold', bold);
            if (node_fs_1.default.existsSync(ital))
                doc.registerFont('Calibri-Italic', ital);
...
function renderHistoricoPdf(opts) {
    ...
    function desenharCabecalho() {
        ...
        if (logoExiste) {
            ...
            if (faculdade.cnpj) {
                doc.text(`CNPJ ${faculdade.cnpj}${faculdade.email ? ' / E-mail.: ' + faculdade.email : ''}${faculdade.telefone ? ' ' + faculdade.telefone : ''}`, textoX, y, { width: textoWidth, align: 'left' });
                y = doc.y;
            }
            ...
            doc.font(F_REG()).fontSize(8).fillColor('#000000');
            if (faculdade.cnpj) {
                doc.text(`CNPJ ${faculdade.cnpj}${faculdade.email ? ' / E-mail.: ' + faculdade.email : ''}${faculdade.telefone ? ' ' + faculdade.telefone : ''}`, MARGEM, y, { width: utilizavel, align: 'center' });
                y = doc.y;
            }
            if (faculdade.endereco) {
    ...
    function garantirEspaco(altura) {
        if (estado.y + altura > bottomLimit)
            estado.y = novaPagina();
    }
    ...
    const notas = disciplinas.map((d) => parseNota(d.nota)).filter((n) => n !== null);
    ...
    if (qrBuffer) {
        ...
        doc.text(`Código de verificação: ${codigoVerificacao}`, MARGEM, estado.y + qrSize + 2, { width: utilizavel, align: 'center' });
...
function desenharCaixaDados(doc, x, yIn, largura, linhas, novaPagina, bottomLimit) {
    const colunas = 4;
...
function desenharTabelaDisciplinas(doc, x, yIn, largura, disciplinas, novaPagina, bottomLimit) {
    ...
    for (const d of disciplinas) {
        if (!periodos.has(d.periodo))
            periodos.set(d.periodo, []);
...
function mover(_event, id, direcao) {
    const db = (0, database_1.getDb)();
    const atual = db.prepare('SELECT * FROM historico_disciplinas WHERE id = ?').get(id);
    if (!atual)
        return { ok: false, error: 'Disciplina não encontrada' };
    // encontra a disciplina adjacente no mesmo período
    const vizinho = direcao === 'up'
        ? db.prepare('SELECT * FROM historico_disciplinas WHERE aluno_id = ? AND periodo = ? AND ordem < ? ORDER BY ordem DESC LIMIT 1').get(atual.aluno_id, atual.periodo, atual.ordem)
        : db.prepare('SELECT * FROM historico_disciplinas WHERE aluno_id = ? AND periodo = ? AND ordem > ? ORDER BY ordem ASC LIMIT 1').get(atual.aluno_id, atual.periodo, atual.ordem);
    if (!vizinho)
        return { ok: false, error: 'Não há disciplina para mover nesta direção' };
    // troca as ordens
...
async function gerarXmlHistorico(event, alunoId) {
    const sessao = (0, auth_1.getSessao)();
    if (!sessao)
        return { ok: false, error: 'Não autenticado' };
    const db = (0, database_1.getDb)();
...
`;
    ...
    node_fs_1.default.writeFileSync(destino.filePath, xml, 'utf8');
    return { ok: true, data: { xmlPath: destino.filePath } };
}
function registrarHistoricoHandlers() {
    electron_1.ipcMain.handle(types_1.IPC_CHANNELS.HISTORICO_LISTAR, (0, auth_1.requerAuth)(listar));
    electron_1.ipcMain.handle(types_1.IPC_CHANNELS.HISTORICO_CRIAR, (0, auth_1.requerAuth)(criar));
    electron_1.ipcMain.handle(types_1.IPC_CHANNELS.HISTORICO_ATUALIZAR, (0, auth_1.requerAuth)(atualizar));
    electron_1.ipcMain.handle(types_1.IPC_CHANNELS.HISTORICO_EXCLUIR, (0, auth_1.requerAuth)(excluir));
    electron_1.ipcMain.handle(types_1.IPC_CHANNELS.HISTORICO_GERAR_PDF, (0, auth_1.requerAuth)(gerarPdf));
    electron_1.ipcMain.handle(types_1.IPC_CHANNELS.HISTORICO_GERAR_XML, (0, auth_1.requerAuth)(gerarXmlHistorico));
    electron_1.ipcMain.handle(types_1.IPC_CHANNELS.HISTORICO_MOVER, (0, auth_1.requerAuth)(mover));
}
