...
exports.iniciarServicoVerificacao = iniciarServicoVerificacao;
...
const API_KEY = config_1.CONFIG.VERIFICACAO_API_KEY;
const PORT = 3001;
const LOCAL_IP = (0, network_1.getLocalIP)();
// Carrega a página de validação (QR com dados embutidos)
let validadorHtml = '';
try {
    const validadorPath = node_path_1.default.join(__dirname, '..', 'resources', 'validador.html');
    if (node_fs_1.default.existsSync(validadorPath)) {
        validadorHtml = node_fs_1.default.readFileSync(validadorPath, 'utf8');
    }
}
catch { /* ignora */ }
let serverInstance = null;
function iniciarServicoVerificacao() {
    if (serverInstance)
    ...
    const server = node_http_1.default.createServer(async (req, res) => {
        ...
        if (req.method === 'POST' && url.pathname === '/api/declaracoes') {
            ...
            req.on('end', () => {
                try {
                    ...
                    db.prepare(`INSERT OR REPLACE INTO declaracoes_web (codigo_verificacao, hash_conteudo, dados_aluno_json, emitido_em)
             VALUES (?, ?, ?, ?)`).run(data.codigo_verificacao, data.hash_conteudo, JSON.stringify(data.dados_aluno || {}), data.emitido_em || new Date().toISOString());
        ...
        if (req.method === 'GET' && url.pathname.startsWith('/v/')) {
            ...
            if (!row) {
                try {
                    const r = db
                        .prepare(`SELECT d.codigo_verificacao, d.hash_conteudo, d.emitido_em,
...
function paginaValido(row) {
    let aluno = {};
    try {
        aluno = JSON.parse(row.dados_aluno_json);
    }
    catch { /* ignora */ }
...
function paginaInvalido(codigo) {
    return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8">
    <meta name="viewport" content="width=device-width,initial-scale=1">
...
//# sourceMappingURL=servico-verificacao.js.map