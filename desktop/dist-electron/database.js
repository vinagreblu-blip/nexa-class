...
exports.gerarCodigoUsuarioUnico = gerarCodigoUsuarioUnico;
exports.getDb = getDb;
...
let db;
function migrarArquivoDb() {
    const novo = (0, config_1.getDbPath)();
    if (node_fs_1.default.existsSync(novo))
        return;
    const candidatos = [(0, config_1.getDbPathAntigo2)(), (0, config_1.getDbPathAntigo)()];
    for (const antigo of candidatos) {
        if (node_fs_1.default.existsSync(antigo)) {
            try {
                node_fs_1.default.renameSync(antigo, novo);
                console.log(`[db] Arquivo migrado: ${node_path_1.default.basename(antigo)} -> nexa-class.sqlite`);
                return;
            }
            catch {
                /* ignora */
...
async function initDatabase() {
    ...
    db = await (0, sqlite_adapter_1.openDatabase)(dbPath);
    createSchema();
    migrateAlunos();
...
function gerarCodigoUsuarioUnico() {
    ...
    const gerar = () => {
        ...
        for (let i = 0; i < 3; i++)
            c += digitos[Math.floor(Math.random() * digitos.length)];
        return c;
    };
    for (let i = 0; i < 50; i++) {
        const cand = gerar();
...
function atribuirCodigosUsuarios() {
    ...
    if (n > 0)
        console.log(`[db] Códigos identificadores atribuídos a ${n} usuário(s)`);
}
function getDb() {
    if (!db) {
...
function migrateAlunos() {
    const cols = db.prepare('PRAGMA table_info(alunos)').all();
    const names = cols.map((c) => c.name);
    const adicionar = (col, definicao) => {
    ...
    const colsDecl = db.prepare('PRAGMA table_info(declaracoes)').all();
...
function seedAdmin() {
    const existing = db
        ...
        .get(config_1.CONFIG.ADMIN_SEED.username);
    if (!existing) {
        const hash = bcryptjs_1.default.hashSync(config_1.CONFIG.ADMIN_SEED.password, 10);
        db.prepare('INSERT INTO usuarios (username, password_hash, nome, role) VALUES (?, ?, ?, ?)').run(config_1.CONFIG.ADMIN_SEED.username, hash, config_1.CONFIG.ADMIN_SEED.nome, 'admin');
        console.log(`[db] Admin inicial criado: ${config_1.CONFIG.ADMIN_SEED.username} (troque a senha após o primeiro login)`);
    }
}
...
function seedDocentes() {
    ...
    if (novos > 0)
        console.log(`[db] Seed de docentes: +${novos} novo(s) (${DOCENTES_SEED.length} no total)`);
}
...
function seedDisciplinas() {
    ...
    for (const d of DISCIPLINAS_SEED) {
        const doc = db.prepare('SELECT id FROM docentes WHERE nome = ?').get(d.docente);
        const r = stmt.run(d.nome, doc?.id ?? null, d.ch);
        if (r.changes > 0)
            novas++;
    }
    if (novas > 0)
        console.log(`[db] Seed de disciplinas: +${novas} nova(s) (${DISCIPLINAS_SEED.length} no total)`);
}
