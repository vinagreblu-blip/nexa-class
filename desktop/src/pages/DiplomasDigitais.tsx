...
function ModalDetalhe({ id, onClose }: { id: number; onClose: () => void }) {
  ...
  const validarArtefato = async (arquivoId: number) => {
    ...
    else setMsg({ tipo: 'erro', texto: r.error ?? 'Falha na validação' });
  };
  ...
  const gerarXml = async (artefato: 'historico_escolar' | 'documentacao_academica') => {
    ...
    setGerando(null);
    if (r.ok) {
      setMsg({ tipo: 'ok', texto: `XML gerado e VÁLIDO contra o XSD oficial 1.05 (schema ${artefato}). Use 'Assinar' para a assinatura digital.` });
    } else {
      // Erros agora vêm com campo+motivo específicos do backend
      ...
      {vista === 'cursos' && (
        <>
          <div className="form-grid">
            ...
            <div>
              <label style={labelStyle}>Carga horária total (h) *</label>
              <input
          ...
          <div style={{ marginTop: 10, padding: '8px 10px', border: '1px solid var(--border)', borderRadius: 8 }}>
            <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 6 }}>
              Reconhecimento via processo e-MEC (usar QUANDO NÃO há ato publicado)
            </div>
            <div className="form-grid">
              <div>
          ...
          <div style={{ marginTop: 10, padding: '8px 10px', border: '1px solid var(--border)', borderRadius: 8 }}>
            <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 6 }}>
              Habilitação (opcional — quando o curso tem habilitações específicas)
            </div>
            <div className="form-grid">
              <div>
                <label style={labelStyle}>Nome da habilitação</label>
                <input value={habilitacao.nome} onChange={(e) => setHabilitacao({ ...habilitacao, nome: e.target.value })} placeholder="Design Gráfico" />
              </div>
              <div>
                <label style={labelStyle}>Data da habilitação (AAAA-MM-DD)</label>
...
// ============================================================
// Modal: Diagn�stico � "Validar Diploma Digital"
// ============================================================
    ...
    <Modal title="Validar Diploma Digital � Diagn�stico" onClose={onClose} width={760}>
      ...
      {(resultado.assinaturas ?? []).map((a: any) => (
        <div key={a.id} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 10, marginTop: 8 }}>
          ...
          <LinhaDiag rotulo="XAdES" ok={a.certDigestOk !== false} detalhe={`SigningTime ${a.signingTime ?? '�'} � CertDigest ${a.certDigestOk == null ? 'ausente' : a.certDigestOk ? 'confere' : 'DIVERGE'}${a.policyId ? ` � Pol�tica: ${a.policyId}` : ''}`} />
          <LinhaDiag
            rotulo="Certificado"
            ok={a.certificado ? a.certificado.validoAgora && a.certificado.usoAssinaturaDigital : null}
            detalhe={
              a.certificado
                ? `${a.certificado.subject} � ${a.certificado.algoritmo} � serial ${a.certificado.serial} � v�lido at� ${String(a.certificado.validoAte).slice(0, 10)}`
                : 'n�o encontrado'
            }
          />
          {a.carimbo ? (
      ...
      {resultado.esqueletos > 0 && (
        <div style={{ marginTop: 10, fontSize: 12, color: 'var(--text-muted)' }}>
          {resultado.esqueletos} posi��o(�es) de assinatura da IES Registradora aguardando (compet�ncia do sistema dela).
        </div>
      )}
